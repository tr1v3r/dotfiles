...
- **dsh-quote-followup 插件**（2026-09-07，独立仓库 `~/workspace/dsh-quote-followup`
  （github.com/tr1v3r/dsh-quote-followup，master 分支）；两个 profile 依赖 npm 版 `^0.2.0`）：「选中对话内容→针对性追问」。
  TUI 面：`session/event` 缓冲 + `Ctrl+Alt+Q` 选择器（tuiDialogs/tuiShortcuts，仅需
  插件激活、无需 Component admission；0.2.0 起选择器第一行是「📋 划选/剪贴板」行——
  骑 TUI 自带 copy-on-select：划选自动进剪贴板，选择器读 pbpaste/wl-paste/xclip/xsel
  引用之，与 web 面体验对齐）+ 经 dsh.nvim 注入套接字
  ...
  extensions 行 + 注入 socket 全链路）。已发布 npm（0.2.0，2026-09-07），两个 profile 走 `^0.2.0`；
  改动源码后 `npm version <patch|minor> && npm publish` 再到 profile 里改版本号 +
  `pnpm clean --lockfile && pnpm install --no-frozen-lockfile`（⚠️ 上一版的锁文件条目会
  在发布几小时后落进 minimumReleaseAge 年龄窗，直接 update 会被供应链策略拒绝，
  必须清锁重解；pnpm-workspace.yaml 的 exclude 也要同步换新版本号）。
